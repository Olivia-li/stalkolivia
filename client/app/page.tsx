...
const LocationMap = () => {
  ...
  useEffect(() => {
    ...
    const fetchMarathonRoute = async () => {
      try {
        const response = await fetch('/tracks.geojson'); // Replace with the path to your GeoJSON file or endpoint
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
    ...
    fetchLocation();
    fetchMarathonRoute();
    const interval = setInterval(fetchLocation, 10000); // Fetch new location every 10 seconds
    ...
    <div style={{ height: '100vh', width: '100%' }}>
      {location && marathonRoute && (
        ...
        >
          ...
          <GeoJSON data={marathonRoute} style={() => ({
            color: "#ff7800",
            weight: 5,
            opacity: 0.65
          })} />
